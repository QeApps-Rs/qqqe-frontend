                        ...
                        <div className="mb-4 border-b border-black border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark rounded-lg">
                          ...
                          <div className="mb-8 p-3">
                            ...
                            <div className="mb-4 border-b border-black pb-5">
                              ...
                              <Radio jsonData={timingOptions} onChange={onTimingChange} />
                              {selectedTiming == 'Based on rules' && <div className="mb-4 mt-3">
                                <div className="text-sm font-semibold">Select the rules</div>
                                <div className="mb-4.5 mt-3">
                                  <Checkbox
                                    key={1}
                                    label='When visitor is exiting the page'
                                    checked={checkedrules?.settings?.existing_page?.is_selected}
                                    onChange={() =>
                                      setRules((prevState) => ({
                                        ...prevState,
                                        settings: {
                                          ...prevState.settings,
                                          existing_page: {
                                            ...prevState.settings.existing_page.is_selected,
                                            is_selected: !checkedrules?.settings?.existing_page?.is_selected
                                          }
                                        }
                                      }))
                                    }
                                ...
                                <div className="mb-4.5">
                                  <Checkbox
                                    ...
                                    checked={checkedrules?.settings?.after_delay_time.is_selected}
                                    onChange={() =>
                                      setRules((prevState) => ({
                                        ...prevState,
                                        settings: {
                                          ...prevState.settings,
                                          after_delay_time: {
                                            ...prevState.settings.after_delay_time.is_selected,
                                            is_selected: !checkedrules?.settings?.after_delay_time.is_selected,
                                            key: 'seconds',
                                            value: ''
                                          }
                                        }
                                  ...
                                  />
                                  {checkedrules?.settings?.after_delay_time.is_selected && <div className="ml-9">
                                    <div>Show again after</div>
                                    <input
                                      type="number"
                                      value={checkedrules?.settings?.after_delay_time?.value ? checkedrules?.settings?.after_delay_time?.value : 0}
                                      onChange={(event) => setRules((prevState) => ({
                                        ...prevState,
                                        settings: {
                                          ...prevState.settings,
                                          after_delay_time: {
                                            ...prevState.settings.after_delay_time.value,
                                            is_selected: checkedrules?.settings?.after_delay_time.is_selected,
                                            key: 'seconds',
                                ...
                                <div className="mb-4.5">
                                  <Checkbox
                                    ...
                                    checked={checkedrules?.settings?.after_scroll_distance.is_selected}
                                    onChange={() =>
                                      setRules((prevState) => ({
                                        ...prevState,
                                        settings: {
                                          ...prevState.settings,
                                          after_scroll_distance: {
                                            ...prevState.settings.after_scroll_distance,
                                            is_selected: !checkedrules?.settings?.after_scroll_distance.is_selected
                                          }
                                        }
                                      }))
                                    }
                                  />
                                  {checkedrules?.settings?.after_scroll_distance.is_selected &&
                                    <div className="ml-9">
                                      <div>Scroll distance</div>
                                      <input
                                        type="number"
                                        value={checkedrules?.settings?.after_scroll_distance?.value}
                                        onChange={(event) =>
                                          setRules((prevState) => ({
                                            ...prevState,
                                            settings: {
                                ...
                                <div className="mb-4.5">
                                  <Checkbox
                                    ...
                                    checked={checkedrules?.settings?.after_pages_visit.is_selected}
                                    onChange={() =>
                                      setRules((prevState) => ({
                                        ...prevState,
                                        settings: {
                                          ...prevState.settings,
                                          after_pages_visit: {
                                            ...prevState.settings.after_pages_visit,
                                            is_selected: !checkedrules?.settings?.after_pages_visit.is_selected
                                          }
                                        }
                                      }))
                                    }
                                  />
                                  {checkedrules?.settings?.after_pages_visit.is_selected && <div className="ml-9">
                                    <div>After</div>
                                    <input
                                      type="number"
                                      value={checkedrules?.settings?.after_pages_visit?.value}
                                      onChange={(event) =>
                                        setRules((prevState) => ({
                                          ...prevState,
                                          settings: {
      ...
      <div className="w-3/4 float-right p-0 h-[83.90vh]">
        <div className="flex justify-between p-4 pl-10 pr-10 border-l border-[#eaedef] items-center flex-wrap w-full bg-white shadow-[6px_0px_7px_#ccc]">
          ...
          <div className="flex">
            ...
            <a className={`rounded-r-md text-lg border border-[#ccc] -ml-px text-black leading-[22px]  ${isView === 'Mobile' ? 'bg-[#eaedef]' : ''} p-1.5 px-2.5`} href="#" onClick={() => setView('Mobile')}>
              <i className="fa fa-mobile" aria-hidden="true"></i>
            </a>
          </div>
        </div>

        {isView === 'Desktop' ? <div className="bg-black/75 h-full p-2 flex items-center justify-center">
