...
const LineChartDashedData = ({
  ...
  useEffect(() => {
    if (orderData && Object.keys(orderData).length > 0) {
      ...
      const totalOrderValues = Object.values(orderData).map(
        (data) => parseFloat(data.total_order_count) || null
      );
      const averageOrderValues = Object.values(orderData).map(
        (data) => parseFloat(data.average_order_count) || null
      );
