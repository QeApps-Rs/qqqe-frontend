...
const ProblemStatements = () => {
  ...
  const classObject = {
    state_div:
      "lg:mr-2 mr-0 lg:mb-0 mb-2 pb-0.5 font-medium outline-none transition duration-300 text-gray-800 hover:text-blue-700 focus:text-blue-700",
    badge_div:
  ...
  <span
    className={`bg-blue-500 ${classObject.badge_label} `}
  >
    Total Suggestions
  </span>
  <span
    className={`bg-success ${classObject.badge_label} `}
  >
    Applied Suggestions
  </span>
  <span
    className={`bg-danger ${classObject.badge_label} `}
  >
    Pending Suggestions
  </span>
  <span
    className={`bg-[#637381] ${classObject.badge_label} `}
  >
    Last Applied Date
  </span>
</div>
          ...
          <div className="flex flex-wrap gap-4">
            {questionData && questionData.length > 0 ? (
              questionData.map((data) => (
                ...
                >
                  <div className="flex lg:flex-row flex-col lg:space-y-0 space-y-4 h-full justify-between lg:items-center items-start">
                    <Link
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <div className="badge flex space-x-2 rounded-full">
                          ...
                          {data.last_applied_date && (
                            <button
                              className={`bg-gray-600 text-white px-2 py-1 rounded-full ${classObject.badge_div}`}
                            >
                              {data.last_applied_date}
                            </button>
                          )}
                    ...
                    <div className="flex items-center lg:justify-normal justify-between lg:w-auto w-full ">
                      <div
                        className={`${classObject.priority} ${getPriorityClass(
                          data.priority
                        )} text-sm font-semibold`}
                      >
                        {data.priority}
                      </div>
                      <div title={data.tooltip_text} className="cursor-pointer">
